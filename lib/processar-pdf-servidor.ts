...
export const processarPdfServidor = async (
  buffer: Buffer,
): Promise<TextoPdf> => {
  ...
  for (let i = 1; i <= numpages; i++) {
    ...
    if (
      textoPagina.replace(/\s/g, "").length < MIN_CARACTERES_UTEIS_POR_PAGINA
    ) {
  ...
  if (paginasParaOcr.length > 0) {
    ...
    try {
      for (const numeroPagina of paginasParaOcr) {
        ...
        const canvasEContexto = canvasFactory.create(
          viewport.width,
          viewport.height,
        );
        ...
        const imagemBuffer = canvasEContexto.canvas.toBuffer(
          "image/png",
        ) as Buffer;
